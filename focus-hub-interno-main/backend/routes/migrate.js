...
router.get('/run', async (req, res) => {
    console.log('[Migrate API] Manual migration triggered - Version: v2-fix-whatsapp');
    try {
        const queries = [
            // Focus Links
            ...
            // WhatsApp Integration (Missing Columns Fix)
            'ALTER TABLE users ADD COLUMN IF NOT EXISTS whatsapp VARCHAR(20)',
            `ALTER TABLE users ADD COLUMN IF NOT EXISTS whatsapp_notifications JSONB DEFAULT '{"tasks": true, "reminders": true, "daily_summary": true, "posts": false}'`,
            'ALTER TABLE users ADD COLUMN IF NOT EXISTS whatsapp_dnd_start TIME',
            'ALTER TABLE users ADD COLUMN IF NOT EXISTS whatsapp_dnd_end TIME'
        ];
