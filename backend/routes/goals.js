...
router.post('/', async (req, res) => {
    const { title, description, progress, current, target, metric, status, dueDate, sector, period, type, userId } = req.body;
    ...
    try {
        ...
        const result = await pool.query(
            `INSERT INTO goals (id, title, description, current_value, target_value, metric, status, due_date, sector, period, type, user_id, created_at)
             ...
             RETURNING *`,
            [id, title, description, current || 0, target || 0, metric, status, dueDate, sector, period, type, userId]
        );
